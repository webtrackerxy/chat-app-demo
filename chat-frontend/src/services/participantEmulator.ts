...
export class ParticipantEmulator {
   ...
   */
  async emulateResponse(conversationId: string, userMessage: string, userName: string): Promise<void> {
    ...
    if (userName === 'System' || userName.startsWith('bot-') || EMULATED_PARTICIPANTS.some(p => p.name === userName)) {
    ...
    console.log('🤖 Selected participants to respond:', respondingParticipants.map(p => p.name));
  ...
  private scheduleResponse(conversationId: string, participant: EmulatedParticipant, userMessage: string): void {
  ...
  private generateResponse(participant: EmulatedParticipant, userMessage: string): string {
    ...
    if (lowerMessage.includes('thanks') || lowerMessage.includes('thank you')) {
      return participant.personality === 'friendly' 
        ? "You're welcome! 😊"
        : "No problem!";
    }
    
    if (lowerMessage.includes('technical') || lowerMessage.includes('code') || lowerMessage.includes('api')) {
  ...
  private async sendEmulatedMessage(conversationId: string, participant: EmulatedParticipant, text: string): Promise<void> {
    try {
  ...
  stopEmulation(conversationId: string): void {
    const keys = Array.from(this.activeTimeouts.keys()).filter(key => key.includes(conversationId));
    keys.forEach(key => {
