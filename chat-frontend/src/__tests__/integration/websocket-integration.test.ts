...
describe('WebSocket Integration Tests', () => {
  ...
  describe('Real-time messaging flow', () => {
    ...
    it('should handle read receipts from multiple users', () => {
      ...
      expect(readReceiptsResult.current.getReadStatusText([readReceipts[0]])).toBe(
        'Read by User One',
      )
      expect(readReceiptsResult.current.getReadStatusText([readReceipts[0], readReceipts[1]])).toBe(
        'Read by User One and User Two',
      )
