...
import { MessageReaction } from '@chat-types'
...
export const useMessageReactions = ({
  ...
  useEffect(() => {
    ...
    const handleReactionAdded = (data: { messageId: string; reaction: MessageReaction }) => {
      ...
      setReactions((prev) => {
        ...
        console.log(
          'FRONTEND: Current reactions for message',
          data.messageId,
          ':',
          messageReactions,
        )
        ...
        if (!existingReaction) {
          ...
          console.log(
            'FRONTEND: Updated reactions for message',
            data.messageId,
            ':',
            updatedReactions,
          )
    ...
    const handleReactionRemoved = (data: {
      ...
      setReactions((prev) => {
        ...
        const updatedReactions = messageReactions.filter((r) => r.id !== data.reactionId)
  ...
  const toggleReaction = useCallback(
    (messageId: string, emoji: string) => {
      if (!isEnabled || !socketService.isConnected()) {
        console.log(
          'Cannot toggle reaction: enabled=',
          isEnabled,
          'connected=',
          socketService.isConnected(),
        )
  ...
  const hasUserReactedWithAny = useCallback(
    (messageId: string) => {
      const messageReactions = reactions.get(messageId) || []
  ...
  const getUserReactionEmoji = useCallback(
    (messageId: string) => {
      const messageReactions = reactions.get(messageId) || []
  ...
  const initializeReactions = useCallback(
    (messageId: string, initialReactions: MessageReaction[]) => {
      setReactions((prev) => {
        const newMap = new Map(prev)
