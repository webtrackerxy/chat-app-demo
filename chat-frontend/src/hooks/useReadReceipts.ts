...
import { ReadReceipt } from '../../../chat-types/src';
...
export const useReadReceipts = ({
  ...
  const autoMarkAsRead = useCallback((messageId: string, delay: number = 1000) => {
    if (!isEnabled || !socketService.isConnected()) return;
  ...
  const getReadStatusText = useCallback((readBy: ReadReceipt[] = []) => {
  ...
  const hasCurrentUserRead = useCallback((readBy: ReadReceipt[] = []) => {
