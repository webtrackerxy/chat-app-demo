...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    const allowedTypes = [
      ...
      'video/mp4', 'video/quicktime', 'video/mpeg', 'video/webm', 'video/avi', 'video/mov', 'video/x-msvideo',
  ...
  describe('File Upload Endpoint', () => {
    ...
    it('should upload an audio file successfully', async () => {
      ...
      if (!fs.existsSync(testAudioPath)) {
        ...
        const m4aHeader = Buffer.from([0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70]);
    ...
    it('should reject unsupported file types', async () => {
      const testFilePath = path.join(__dirname, 'fixtures', 'test-script.js');
